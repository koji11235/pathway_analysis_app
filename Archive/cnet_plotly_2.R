...
calculate_gene_color(categories, categories2color)
...
gene2color_df <- as.data.frame(get.edgelist(g)) %>% 
  ...
  arrange(desc(n)) 
gene2color <- gene2color_df%>% 
  pull(gene_color)
names(gene2color) <- gene2color_df%>% 
  pull(V2)

names(V(g))
names(V(g))[n+1:length(V(g))]
gene2color[names(V(g))[(n+1):length(V(g))]] %>% length()
...
as.data.frame(get.edgelist(g)) %>% 
  ...
  summarise(categories=str_c(V1, collapse  = ";"),
            n=n())  %>% 
  arrange(desc(n)) %>% 
  mutate(test = categories %>% str_split(";") %>% str_c(sep  = "_"))

as.data.frame(get.edgelist(g)) %>% 
  group_by(V2) %>% 
  summarise(categories_=list(V1),
            n=n())  %>% 
  arrange(desc(n)) %>% 
  mutate(test = gene2color(categories_, categories2color))
...
as.data.frame(get.edgelist(g)) %>% 
  ...
  summarise(categories_=list(V1),
            n=n(),
            gene_color = gene2color(V1, categories2color))  %>% 
  arrange(desc(n)) 



names(geneSets) %>% factor() %>% as.numeric()
categories2color <- category_color_palette[as.numeric(factor(names(geneSets)))]
names(categories2color) <- names(geneSets)

categories = factor(c("Signaling by Receptor Tyrosine Kinases", "Assembly of collagen fibrils and other multimeric structures"))

result_color = "#000000"
for (category in categories){
  result_color <- mapply(max, 
                         col2rgb(categories2color[[category]])[,1], 
                         col2rgb(result_color)[,1])
}

gene2color <- function(categories, categories2color){
  result_color <- "#000000"
  for (category in categories){
    result_color_rgb <- mapply(max, 
                           col2rgb(categories2color[[category]])[,1], 
                           col2rgb(result_color)[,1])
    result_color <- rgb(red = result_color_rgb["red"], green = result_color_rgb["green"], blue = result_color_rgb["blue"], maxColorValue = 255)
    
  }
  return(result_color)
}

gene2color(categories, categories2color)
...
plot_ly(x = ~Xn, y = ~Yn,
  ...
  add_segments_different_color(x=edge_df$x,  xend=edge_df$xend,
                               ...
                               color = edge_df$category_color) %>% 
  add_markers(showlegend=T) %>% 
  add_text(x = ~Xn, y = ~Yn, text = label, textposition = "bottom center",
           textfont = list(size = vs$size, family = 'Helvetica') ,inherit = F) 
...
es %>% 
  group_by(V2) %>% 
  summarise(test=str_c(V1, collapse  = ";"),
            n=n())  %>% 
  arrange(desc(n) ) ->a
...
color <- (col2rgb(category_color_palette[[1]]) + col2rgb(category_color_palette[[2]])  ) /2
a[,1]["red"]
color_code <- rgb(red = color[,1]["red"], green = color[,1]["green"], blue = color[,1]["blue"], maxColorValue = 255)
barplot(c(2, 5, 3, 4, 6), col = c(category_color_palette, color_code))


library(grDevices)
col2rgb(category_color_palette[[1]]) %>% rgb2hsv()
rgb2hsv(col2rgb(category_color_palette[[1]]))
hsv_value <- (rgb2hsv(col2rgb(category_color_palette[[1]])) + rgb2hsv(col2rgb(category_color_palette[[2]]))  ) 

color_code_hsv <- hsv(h = hsv_value["h",1], hsv_value["s",1], hsv_value["v",1])

scatterplot(1,1, col="grey")

max(col2rgb(category_color_palette[[1]]) , col2rgb(category_color_palette[[2]])) 

color <- mapply(max, col2rgb(category_color_palette[[1]])[,1], col2rgb(category_color_palette[[2]])[,1])
color_code <- rgb(red = color["red"], green = color["green"], blue = color["blue"], maxColorValue = 255)


ggplot()+
  geom_point(aes(x=1, y=1),
             size=100, color=category_color_palette[[1]], alpha=0.5)+
  geom_point(aes(x=1, y=1),
             size=100, color=category_color_palette[[2]], alpha=0.5)+
  geom_point(aes(x=-1, y=1),
             size=100, color=category_color_palette[[1]], alpha=1)+
  geom_point(aes(x=-1, y=-1),
             size=100, color=category_color_palette[[2]], alpha=1)+
  geom_point(aes(x=1, y=-1),size=100, color=color_code[[1]], alpha=1)



ggplot()+
  geom_point(aes(x=1, y=1),
             size=100, color="#FCC2A5", alpha=1)+
  geom_point(aes(x=0, y=0),
             size=100, color="#66C2A5", alpha=1)+
  geom_point(aes(x=0, y=1),
             size=100, color="#FC8D62", alpha=1)+
  geom_point(aes(x=1, y=0),
             size=100, color="#668D62", alpha=1)+
  geom_point(aes(x=0.5, y=0.5),
             size=100, color="#B6FCAF", alpha=1)+
  geom_point(aes(x=0.5, y=0.5),
             size=100, color="#A1DF9B", alpha=1)
#668D62
rgb(red = 255, green = 255, blue = 255, maxColorValue = 255)
category_color_palette <- brewer.pal(8, "Set2")

col2rgb("darkgrey")
rgb2hsv()
col2rgb("#668D62") %>% rgb2hsv() -> a
a["v",1] <- 0.7607843
col2rgb("#FCC2A5") %>% rgb2hsv()
col2rgb("#66C2A5") %>% rgb2hsv()
hex(a)
hsv(a["h",1],a["s",1],a["v",1])
hsvs = lapply(brewer.pal(8, "Set2"), function(color){rgb2hsv(col2rgb(color))}) 
v_list =c()
for (hsv in hsvs){
  v_list <- c(v_list, hsv["v",1])
}
