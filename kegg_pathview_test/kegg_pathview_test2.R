...
pathID <- enrichment_result$ID[[1]]
genes <- enrichment_result$geneID[[1]] %>% str_split("/") %>% .[[1]]
pv.out <- mypathview(gene.data = genes, pathway.id = pathID)

#outfile <- paste( tempFolder,"/",pathID,".",randomString,".png",sep="")

outfile <- paste( getwd(),"/",pathID,".","pathview",".png",sep="")
